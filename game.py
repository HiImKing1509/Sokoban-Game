...
class Game:
    def __init__(self, window):
        ...
        self.load_textures()
        self.player = None
        self.index_level = 2 # replace 1 to 2
        self.load_level()
        self.play = True
    ...
    def process_event(self, event):
        ...
        if event.type == KEYDOWN:
            ...
            if event.key in [K_UP, K_DOWN, K_LEFT, K_RIGHT, K_z, K_s, K_q, K_d]:
                ...
                if self.has_win():
                    self.index_level += 1
                    if (self.index_level == 20): # Replace 19 to 17
                        self.index_level = 1
                    self.scores.save()
                    self.load_level()
            if event.key == K_r:
...
# auto move 1 fucntion: DFS, BFS, UCS
    def auto_move(self):

        # Tạo list chứa kết quả của từng giải pháp
        list_strategy = []
        print('Level {}'.format(self.index_level))

        # Thêm kết quả DFS vào list
        strategy = get_move(self.level.structure[:-1], self.level.position_player, 'dfs')
        list_strategy.append(strategy)
        
        # Thêm kết quả BFS vào list
        strategy = get_move(self.level.structure[:-1], self.level.position_player, 'bfs')
        list_strategy.append(strategy)

        # Thêm kết quả UCS vào list
        strategy = get_move(self.level.structure[:-1], self.level.position_player, 'ucs')
        list_strategy.append(strategy)

        list_algorithm = ['DFS', 'BFS', 'UCS']

        # Tạo vòng lặp lưu trữ tất cả file kết quả cho mỗi giải pháp của mỗi level (định dạng file .txt)
        for i in range (0, len(list_strategy), 1):
            with open("assets/sokobanSolver/Solverlevel_" + str(self.index_level) + ".txt", 'a') as solver_file:
                solver_file.write('[Sequence Action ' + list_algorithm[i] + '] = ')
                solver_file.write('%s, \n' % list_strategy[i][0])
                solver_file.write('[Length of path for ' + list_algorithm[i] + '] = ')
                solver_file.write('%s, \n' % len(list_strategy[i][0]))
                solver_file.write('[Time action ' + list_algorithm[i] + '] = %s,'  % list_strategy[i][1])
                solver_file.write('\n\n')
...
# auto move 2 function: BFS, UCS
    # def auto_move(self):

    #     # Tạo một list chứa kết quả của từng giải pháp
    #     list_strategy = []
    #     print('Level {}'.format(self.index_level))
    #     # strategy = get_move(self.level.structure[:-1], self.level.position_player, 'dfs')
    #     # list_strategy.append(strategy)
    ...
    #     list_algorithm = ['BFS', 'UCS']

    #     for i in range (0, len(list_strategy), 1):
    #         with open("assets/sokobanSolver_BFS_UCS/Solverlevel_" + str(self.index_level) + ".txt", 'a') as solver_file:
    #             solver_file.write('[Sequence Action ' + list_algorithm[i] + '] = ')
    #             solver_file.write('%s, \n' % list_strategy[i][0])
    #             solver_file.write('[Path length of ' + list_algorithm[i] + '] = ')
    #             solver_file.write('%s, \n' % len(list_strategy[i][0]))
    #             solver_file.write('[Time action ' + list_algorithm[i] + '] = %s,'  % list_strategy[i][1])
    #             solver_file.write('\n\n')
